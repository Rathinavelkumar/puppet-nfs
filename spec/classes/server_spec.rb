...
describe 'ol8nfs::server' do
    let(:facts) { {:operatingsystem => 'ubuntu', :concat_basedir => '/tmp', } }
    it do
      should contain_concat__fragment('ol8nfs_exports_header').with( 'target' => '/etc/exports' )
    end
    context "ol8nfs_v4 => true" do
      let(:params) { {:ol8nfs_v4 => true, } }
      it do
        should contain_concat__fragment('ol8nfs_exports_root').with( 'target' => '/etc/exports' )
  ...
  context "operatingsysten => ubuntu" do
    let(:facts) { {:operatingsystem => 'ubuntu', :concat_basedir => '/tmp', } }
    it { should contain_class('ol8nfs::server::ubuntu') }
  end
  ...
  context "operatingsysten => gentoo" do
    ...
    it { should contain_class('ol8nfs::server::gentoo') }
  end
  context "operatingsysten => darwin" do
    let(:facts) { {:operatingsystem => 'darwin'} }
    it do
      expect {
        should contain_class('ol8nfs::server::darwin')
      }.to raise_error(Puppet::Error, /ol8nfs server is not supported on Darwin/)
    end
  end
end
